...
| input_price | Standard realtime API input token price in USD / 1M tokens. |
| output_price | Standard realtime API output token price in USD / 1M tokens. |
| cached_input_price | Cached input price when officially available; missing is allowed but must stay missing. |
| price_unit | Must be `USD / 1M tokens`. |
