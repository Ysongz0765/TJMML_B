...
正式结果显示，成本与场景效用之间不存在可跨场景复用的单一排序。DeepSeek-
...
函数形式判断。因此，当前横截面样本未提供稳定证据支持统一的成本边际
收益递减关系。

本轮最重要的发布限制是价格观测完整性。Claude Fable 5 的 base SKU 价格
不能替代带 fallback 的完整配置成本，GLM-5.2 也不能用相邻 SKU 或第三方
报价补齐。Claude Fable 5 benchmark 配置包含不可完整追溯的 fallback 成本，
不能与完整成本样本进行严格等价比较；GLM-5.2 因缺少可复核的公开标准 API
单价，未进入严格成本--效用 Pareto 主分析。当前结果限定于标准化工作负载、
已核验价格和 FULL 主 cohort。
