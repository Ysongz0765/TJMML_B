...
def main() -> None:
    ...
    medians = matrix_a.median()
    kimi_gap = pd.DataFrame({"dimension": matrix_a.columns, "kimi_score": kimi.values, "median_score": medians.values})
    ...
    expected_zero = set(matrix_a.index[matrix_a["C5"] == 0])
    if expected_zero != TYPE_A_MODELS:
        raise RuntimeError(f"C5 effective zeros do not equal Type-A set: {expected_zero}")
    ...
    stability_md = "\n".join(
        [
            ...
            "",
            old_audit.to_markdown(index=False),
            "",
